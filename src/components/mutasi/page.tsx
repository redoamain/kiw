...
const DataMutasiPage: React.FC = () => {
  ...
  const { data, loading, error } = useSelector(
    (state: RootState) => state.mutasi
  );

  const [selectedRows, setSelectedRows] = useState<MutasiType[]>([]);
  ...
  useEffect(() => {
    ...
    toast.promise(myPromise, {
      loading: "Memuat data Mutasi...",
      success: "Data Mutasi berhasil dimuat!",
      error: "Gagal memuat data Mutasi",
    });
  }, [dispatch]);
  ...
  const handleExport = () => {
    ...
    const fileName = `Data_Mutasi_${searchTerm ? searchTerm : ''}_${new Date().toISOString().split('T')[0]}${EXCEL_EXTENSION}`;
  ...
  const handleDateRangeChange = (
    ...
    const myPromise = dispatch(fetchMutasiData({ startDate, endDate }));
    toast.promise(myPromise, {
  ...
  // Statistik tambahan berdasarkan field yang mungkin ada di data Mutasi
    ...
    <div className="flex items-center justify-center min-h-screen">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          <p className="text-muted-foreground">Terjadi kesalahan saat memuat data Mutasi:</p>
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Data Mutasi</h1>
            <p className="text-muted-foreground">
              Kelola dan pantau seluruh data Laporan Barang Mutasi antar Gudang
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Semua laporan Mutasi</p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filter data Mutasi berdasarkan kriteria tertentu
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-medium">Pencarian Data Mutasi</label>
              </div>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                <Input
                  type="search"
                  placeholder="Cari berdasarkan nomor Mutasi, supplier, atau nama barang..."
                  value={searchTerm}
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
            <div className="text-sm text-muted-foreground">
              {searchTerm 
                ? `Menampilkan ${filteredItems} dari ${totalItems} data Mutasi (setelah pencarian)`
                : `Menampilkan ${totalItems} data Mutasi`
              }
            </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <CardTitle>Daftar Data Mutasi</CardTitle>
              <CardDescription>
          ...
          {data.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-semibold">Tidak ada data Mutasi</h3>
                <p className="text-muted-foreground max-w-md">
                  Data Mutasi kosong. Coba refresh!!
                </p>
              </div>
